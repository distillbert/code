...
class BERTDataset(Dataset):
    ...
    def get_corpus_line(self, item):
        ...
        if self.on_memory:
            t1 = self.all_docs[item]
            # t2 = self.all_docs[sample["doc_id"]][sample["line"]+1]
            # used later to avoid random nextSentence from same doc
            self.current_doc = 10000000
            return t1, 0
        else:
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained(args.model_file, do_lower_case=True)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                prediction_scores, _ = model(input_ids, segment_ids, input_mask, lm_label_ids)
                loss = loss_fct(prediction_scores.view(-1, model.module.config.vocab_size), lm_label_ids.view(-1)).mean()
                if n_gpu > 1:
